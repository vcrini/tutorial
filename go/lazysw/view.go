...
func (m model) View() string {
	...
	if m.focusedPanel == 2 {
		...
		if len(filtered) == 0 {
			...
			details.WriteString(fmt.Sprintf("Ruolo: %s  Taglia: %d  Wild Card: %s\n", mon.Role, mon.Size, wc))
			details.WriteString(fmt.Sprintf("Parata: %s  Robustezza: %s  Ferite max: %d\n", mon.Parry, mon.Toughness, monsterWoundsMax(mon)))
			if mon.Attack.Name != "" {
		...
		if len(m.encounter) == 0 {
			...
			details.WriteString(fmt.Sprintf("Ruolo: %s  Taglia: %d  Wild Card: %s\n", mon.Role, mon.Size, wc))
			details.WriteString(fmt.Sprintf("Parata: %s  Robustezza: %s  Ferite: %d/%d\n", mon.Parry, mon.Toughness, e.Wounds, basePF))
			if mon.Attack.Name != "" {
				bonus := strings.TrimSpace(mon.Attack.Bonus)
				if bonus != "" && !strings.HasPrefix(bonus, "+") && !strings.HasPrefix(bonus, "-") {
					bonus = "+" + bonus
