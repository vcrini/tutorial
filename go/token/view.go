...
func (m model) View() string {
	...
	default:
		...
		rightTop.WriteString(dim.Render("↑↓: seleziona PNG  ←→: token -/+  d/x: elimina  (focus su PNGs)") + "\n\n")
		if len(m.pngs) == 0 {
	...
	if m.appState == createPNGState {
		...
		helpText = "↑↓ seleziona PNG  •  ←→ token  •  d/x elimina  •  1 menu  2 PNGs"
	}
	if totalWidth < 80 {
		if m.appState == createPNGState {
			helpText = "Enter conferma  •  Esc/q annulla"
		} else if m.focusedPanel == 1 {
			helpText = "↑↓ seleziona  •  ←→ token  •  d/x elimina  •  1 menu  2 PNGs"
		} else {
